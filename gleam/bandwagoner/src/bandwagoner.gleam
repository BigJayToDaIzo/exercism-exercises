...
pub fn root_for_team(team: Team) -> Bool {
  case team.coach.name {
    "Gregg Popovich" -> True
    _ -> case team.coach.former_player {
      True -> True
      _ -> case team.name {
        "Chicago Bulls" -> True
        _ -> case team.stats.wins {
          _i if team.stats.wins >= 60 -> True
          _ -> case team.stats.losses {
            _i if team.stats.losses > team.stats.wins -> True
            _ -> False
          }
        }
      }
