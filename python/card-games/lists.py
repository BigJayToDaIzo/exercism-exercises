...
def approx_average_is_average(hand):
    ...
    `true average`?
    """
    firstLastAvg = (hand[0] + hand[-1]) / 2
    median = hand[len(hand) // 2]
    return firstLastAvg == card_average(hand) or median == card_average(hand)


def average_even_is_average_odd(hand):
    """Return if the (average of even indexed card values) == (average of odd
    indexed card values).
