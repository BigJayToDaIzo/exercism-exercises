    ...
    def test_clean_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                error_msg = (
                    f"Expected the ingredient list for {
                        item[0]} to be de-duplicated, "
    ...
    def test_check_drinks(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                error_msg = f"Expected {result} for {
                    item}, but got something else instead."
                self.assertEqual(
    ...
    def test_categorize_dish(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                error_message = f"Expected category {result} for {
                    item[0]}, but got a different category instead."
                self.assertEqual(
    ...
    def test_tag_special_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                error_message = f"Expected {result} for {
                    item}, but got something else instead."
                self.assertEqual(
    ...
    def test_separate_appetizers(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                result_type_error = f"You returned {
                    type(separate_appetizers(item[0], item[1]))}, but a list was expected."
