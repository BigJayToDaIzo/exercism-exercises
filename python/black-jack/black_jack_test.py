            ...
            with self.subTest(f"variation #{variant}", card=card, expected=expected):
                ...
                error_msg = (
                    ...
                    f"but the test expected {
                        expected} as the {card} card value."
                )
        ...
        for variant, (card_one, card_two, expected) in enumerate(test_data, 1):
            with self.subTest(
                ...
                error_msg = (
                    ...
                    f"but the test expected {
                        expected} as the result for the cards {card_one, card_two}."
                )
        ...
        for variant, (hand, expected) in enumerate(test_data, 1):
            with self.subTest(f"variation #{variant}", hand=hand, expected=expected):
                ...
                error_msg = (
                    ...
                    f'The function returned {actual_result}, '
                    f'but hand {hand} {
                        "is" if expected else "is not"} a blackjack.'
                )
        ...
        for variant, (hand, expected) in enumerate(test_data, 1):
            with self.subTest(f"variation #{variant}", input=hand, expected=expected):
                ...
                error_msg = (
                    ...
                    f'The function returned {actual_result}, '
                    f'but hand {hand} {
                        "can" if expected else "cannot"} be split into pairs.'
                )
        ...
        for variant, (hand, expected) in enumerate(test_data, 1):
            with self.subTest(f"variation #{variant}", hand=hand, expected=expected):
                ...
                error_msg = (
                    ...
                    f'The function returned {actual_result}, '
                    f'but hand {hand} {
                        "can" if expected else "cannot"} be doubled down.'
                )
